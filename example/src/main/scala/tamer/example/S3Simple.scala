...
object S3Simple extends App {
  private val myProgram: ZIO[Blocking with Clock with S3 with KafkaConfig, TamerError, Unit] =
    new TamerS3SuffixDateFetcher[Blocking with Clock with S3 with KafkaConfig]().fetchAccordingToSuffixDate(
      bucketName = "myBucket",
  ...
  private lazy val myFullLayer = myS3Layer ++ Config.live
